...
如果实例已在 Vast.ai 网页端删除，本地 `deployment.json` 仍会暂时保留原实例 ID。菜单中的“查看部署状态”或“销毁实例”会先与账户的活动实例列表对账：确认远端已不存在后，将本地状态更新为 `destroyed`，不会重复发送删除请求。Vast API 查询和销毁命令均有超时限制，不会无限等待。
